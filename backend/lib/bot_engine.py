...
def order_window(timeframe: str) -> int:
    return ORDER_WINDOW.get(timeframe, 300)
...
def next_slot(moment: datetime, timeframe: str = "1h") -> datetime:
    ...
    if minutes == 60:
        ...
        future = [slot for slot in session_slots if slot > moment]
        if future:
            return min(future)
        return _valid_hourly_slots_for_day(moment.date() + timedelta(days=1), tz)[0]

    anchor = moment.replace(hour=0, minute=0, second=0, microsecond=0)
    elapsed = (moment - anchor).total_seconds() / 60
    steps = int(elapsed // minutes) + 1
    slot = anchor + timedelta(minutes=steps * minutes)
...
def candle_close_label(candle: dict[str, Any], timeframe_minutes: int) -> str:
    ...
    if timestamp > 10_000_000_000:
        timestamp /= 1000
    start = datetime.fromtimestamp(timestamp, IST)
    end = start + timedelta(minutes=timeframe_minutes)

    def clock_value(value: datetime) -> str:
        hour = value.hour % 12 or 12
        return f"{hour:02d}:{value.minute:02d}"

    period = "pm" if start.hour >= 12 else "am"
    return f"{clock_value(start)} - {clock_value(end)} = {clock_value(start)}{period}"
...
def candle_is_closed(candle: dict[str, Any], timeframe_minutes: int, now: datetime) -> bool:
    ...
    if timestamp > 10_000_000_000:
        timestamp /= 1000
    close_at = datetime.fromtimestamp(timestamp, IST) + timedelta(minutes=timeframe_minutes)
    return close_at <= now
...
def current_slot_boundary(now: datetime, timeframe_minutes: int) -> datetime:
    if timeframe_minutes == 60:
        ...
        cand: list[datetime] = []
        cand.extend(_valid_hourly_slots_for_day(now.date() - timedelta(days=1), tz))
        cand.extend(_valid_hourly_slots_for_day(now.date(), tz))
        valid = [slot for slot in cand if slot <= now]
        if not valid:
            return _valid_hourly_slots_for_day(now.date() - timedelta(days=1), tz)[-1]
        return max(valid)

    anchor = now.replace(second=0, microsecond=0)
    minute = (anchor.minute // timeframe_minutes) * timeframe_minutes
    return anchor.replace(minute=minute)


def tp_sl_for(side: str, entry: float, tp_pct: float, sl_pct: float | None) -> tuple[float, float | None]:
    if side == "buy":
        return entry * (1 + tp_pct / 100), (entry * (1 - sl_pct / 100) if sl_pct else None)
    return entry * (1 - tp_pct / 100), (entry * (1 + sl_pct / 100) if sl_pct else None)


def pnl_pct_for(side: str, entry: float, price: float, leverage: float) -> float:
    move = (price - entry) / entry if side == "buy" else (entry - price) / entry
    return move * 100 * leverage


def price_label(value: float | None) -> str:
    return f"{value:.8f}" if value is not None else "off"
...
def filled_quantity(status: dict[str, Any], fallback: float) -> float:
    ...
    total = status.get("total_quantity") or status.get("quantity")
    remaining = status.get("remaining_quantity")
    if total is not None and remaining is not None:
        return max(float(total) - float(remaining), 0.0)
    return fallback
...
def live_capital_limit(strategy_capital: Decimal, wallet_balance: Decimal) -> Decimal:
    ...
    return max(Decimal("0"), min(strategy_capital, usable))
...
def position_is_open(pos: dict[str, Any] | None) -> bool:
    if not pos:
        return False
    if "is_open" in pos:
        return bool(pos["is_open"])
    status = str(pos.get("status") or "").lower()
    if status:
        return status not in ("closed", "liquidated", "exited")
    return True
...
async def send_order_with_retry(
    ...
    for attempt in range(1, ORDER_RETRY_ATTEMPTS + 1):
        try:
            if market_entry:
                return await trade.place_market(pair, side, quantity, leverage)
            return await trade.place_limit(pair, side, price, quantity, leverage)
        except Exception as exc:
            last_exc = exc
            logger.warning("order attempt %d/%d failed for %s: %s", attempt, ORDER_RETRY_ATTEMPTS, pair, exc)
...
class BotEngine:
    ...
    def state(self) -> BotState:
        return BotState(
            ...
            credentials_configured=all(trade.credentials()),
            timezone="Asia/Kolkata",
            trading_window="05:30 → 03:40 IST · slots follow each strategy's timeframe",
            server_time_ist=now_ist().isoformat(timespec="seconds"),
            in_window=in_window(),
    ...
    def positions(self) -> list[LivePosition]:
        ...
        for s in self.strategies.values():
            ...
            if rt is None or rt.phase not in ("pending_order", "in_position") or not rt.pair:
                continue
            ticker = store.tickers.get(rt.pair)
            last = ticker.last if ticker else None
            entry = rt.entry or 0.0
            side = rt.side or "sell"
            pnl_pct = pnl_pct_for(side, entry, last, rt.leverage) if last and entry else None
            out.append(
                LivePosition(
                    trade_id=rt.trade_id or "",
                    strategy_id=s.id,
                    strategy_name=s.name,
                    pair=rt.pair,
                    symbol=ticker.symbol if ticker else rt.pair,
                    side=side,
                    ...
                    last_price=last,
                    pnl_pct=pnl_pct,
                    pnl_inr=(rt.capital * pnl_pct / 100) if pnl_pct is not None else None,
                    distance_to_tp_pct=((rt.tp - last) / last * 100) if last and rt.tp else None,
                    distance_to_sl_pct=((rt.sl - last) / last * 100) if last and rt.sl else None,
                    opened_at=(rt.slot or now_ist()).isoformat(timespec="seconds"),
                    order_deadline_ist=rt.order_deadline.strftime("%H:%M:%S") if rt.order_deadline else None,
                )
            )
        return out
    ...
    async def _update_trade(self, trade_id: str | None, fields: dict[str, Any]) -> None:
        if not trade_id:
            logger.error("trade update skipped because trade_id is missing: %s", fields)
            return
        for attempt in range(1, DB_RETRY_ATTEMPTS + 1):
            try:
                ...
                if attempt == DB_RETRY_ATTEMPTS:
                    logger.error("trade update failed after retries for %s: %s", trade_id, exc)
                    return
                await asyncio.sleep(DB_RETRY_DELAY)
    ...
    async def load(self) -> None:
        ...
        try:
            ...
            for doc in reversed(log_docs):
                doc.pop("_id", None)
                try:
                    self.logs.append(LogEntry(**doc))
                except Exception as exc:
                    logger.warning("invalid persisted bot log skipped: %s", exc)
        except Exception as exc:
        ...
        try:
            ...
            for doc in docs:
                ...
                if self.bot_on and s.enabled:
                    s.status = "waiting"
                    s.detail = "Armed after restart — waiting for the next slot."
        except Exception as exc:
    ...
    async def _recover_open_trade(self, s: Strategy, rt: Runtime) -> None:
        try:
            ...
            logger.warning("trade recovery lookup failed for %s: %s", s.name, exc)
            return
        if not doc:
            return

        position_id = doc.get("position_id") or ""
        order_id = doc.get("order_id") or ""
        still_open = True
        if trade.live_enabled() and position_id:
            try:
                pos = await trade.position_status(position_id)
                still_open = position_is_open(pos)
            except Exception as exc:
                self.log("error", f"Could not verify {s.name}'s recovered position on the exchange: {exc}", s)

        if not still_open:
            await self._update_trade(doc.get("id"), {
        ...
        rt.quantity = doc.get("quantity") or 0.0
        rt.leverage = doc.get("leverage") or s.leverage
        rt.capital = doc.get("capital_inr") or 0.0
        rt.order_id = order_id or None
        rt.position_id = position_id or None
        rt.phase = "in_position" if doc.get("status") == "open" else "pending_order"
        if rt.phase == "pending_order":
            rt.order_deadline = now_ist() + timedelta(seconds=order_window(s.timeframe))
        s.open_pair, s.open_side = rt.pair, rt.side
        s.entry_price, s.tp_price, s.sl_price = rt.entry, rt.tp, rt.sl
        action = "monitoring" if rt.phase == "in_position" else "awaiting a fill on"
        self._set(s, rt.phase, f"Recovered after restart — {action} {rt.pair}.")
        self.log(
            "info",
    ...
    async def set_enabled(self, sid: str, enabled: bool) -> Strategy | None:
        ...
        if s is None:
            return None
        s.enabled = enabled
        rt = self.runtime.setdefault(sid, Runtime())
        if not enabled:
            rt.phase = "waiting"
            self._set(s, "stopped", "Disabled by operator.")
        else:
            self._set(s, "waiting", "Armed — waiting for the next slot.")
    ...
    async def set_bot(self, on: bool) -> None:
        ...
        self.log("info", f"BOT {'ON' if on else 'OFF'} — execution mode {trade.mode()}.")
        if not on:
            for s in self.strategies.values():
                rt = self.runtime.setdefault(s.id, Runtime())
                if rt.phase not in ("in_position", "pending_order"):
                    rt.phase = "waiting"
    ...
    async def _prescan(self, s: Strategy, rt: Runtime, now: datetime) -> None:
        top = self._ranked(s)
        if not top:
            self._set(s, "waiting", "Waiting for scanner data.")
            return
        rt.candidates = [t.pair for t in top]
        rt.phase = "scanning"
        label = "top losers" if s.coin_pick == "top_loser" else "top gainers"
        self.log(
            "signal",
    ...
    async def _inr_instrument(self, pair: str) -> dict[str, Any]:
        ...
        if not self._inr_pairs:
            try:
                ...
                self._inr_pairs = set()
        if self._inr_pairs and pair not in self._inr_pairs:
            self._inr_detail[pair] = {}
            return {}
        try:
            detail = await trade.instrument_detail(pair, "INR")
        except Exception:
            detail = {}
    ...
    async def _select(self, s: Strategy, rt: Runtime, now: datetime) -> None:
        ...
        for pair in rt.candidates:
            ...
            if side is None:
                ...
                continue
            if not await self._inr_instrument(pair):
                self.log(
                    "signal",
            ...
            picks.append((abs(change), pair, side, float(closed["close"])))

        if not picks:
            rt.phase, rt.slot = "waiting", None
            self._set(s, "waiting", "No tradable candle at this slot — waiting for the next one.")
            self.log("info", "No candidate produced a clear, INR-tradable candle this slot.", s)
            return
    ...
    async def _select_highest_mover(self, s: Strategy, rt: Runtime, now: datetime) -> None:
        ...
        for pair in rt.candidates:
            ...
            if change <= 0:
                continue
            try:
                series = await candle_api.get_candles(pair, s.timeframe, 3)
            except Exception as exc:
                self.log("error", f"{s.timeframe} candle fetch failed for {pair}: {exc}", s)
                continue
            closed = series[:-1]
            ...
            candle = closed[-1]
            volume = float(
                getattr(ticker, "volume_24h", None)
                or getattr(ticker, "quote_volume", None)
                or getattr(ticker, "volume", None)
                or 0.0
            )
            candidates.append((float(change), volume, pair, float(candle["close"])))
            self.log(
                "signal",
    ...
    async def _select_reversal_candidate(self, s: Strategy, rt: Runtime, now: datetime) -> None:
        ...
        max_wait_seconds = 60
        poll_interval = 3
        fresh_closed: dict[str, list[dict[str, Any]]] = {}
        pending = list(rt.candidates[:4])
        waited = 0.0
        ...
        while pending:
            ...
            for pair in pending:
                try:
                    series = await candle_api.get_candles(pair, tf, 4)
                except Exception as exc:
                ...
                if timestamp > 10_000_000_000:
                    timestamp /= 1000
                cn2_close_at = datetime.fromtimestamp(timestamp, IST) + timedelta(minutes=tf_minutes)
            ...
            self.log(
                ...
                + ", ".join(pending) + f" ({int(waited)}s waited).",
                s,
            )
        ...
        for pair, closed in fresh_closed.items():
            candle_1, candle_2 = closed[-2], closed[-1]
            cn1, cn2 = float(candle_1["close"]), float(candle_2["close"])
            cn1_side = candle_side(candle_1)
            cn2_side = candle_side(candle_2)
            cn1_label = "green" if cn1_side == "buy" else "red" if cn1_side == "sell" else "flat"
            cn2_label = "green" if cn2_side == "buy" else "red" if cn2_side == "sell" else "flat"
            ticker = store.tickers.get(pair)
            change = float(ticker.change_pct or 0.0) if ticker else 0.0
            self.log(
                "signal",
            ...
            if not (cn1_side == "buy" and cn2_side == "sell"):
                self.log(
                    "signal",
            ...
            candidates.append(
                (
                    ...
                    pair,
                    cn2,
                    candle_close_label(candle_1, tf_minutes),
                    candle_close_label(candle_2, tf_minutes),
                )
            )

        if not candidates:
            rt.phase, rt.slot = "waiting", None
            self._set(s, "waiting", f"No Top-4 coin matched Green {tf} → Red {tf}.")
            return

        _, pair, _close, green_time, red_time = max(candidates)
        rt.pair = pair
        rt.side = "sell"
        trigger_tf = TRIGGER_TF.get(tf, "1m")
        self.log(
            "signal",
        ...
        rt.trigger_deadline = next_slot(now, tf) - timedelta(minutes=2)
        rt.seen_green_trigger = False
        rt.last_trigger_candle = None
        rt.phase = "trigger_wait"
        self._set(s, "trigger_wait", f"{pair}: fetching {trigger_tf} OHLC — wait for Green then Red close.")

    async def _await_reversal_trigger(self, s: Strategy, rt: Runtime, now: datetime) -> None:
        if not rt.pair:
            self._reset(s, rt)
            return
        trigger_tf = TRIGGER_TF.get(s.timeframe, "1m")
        if rt.trigger_deadline and now >= rt.trigger_deadline:
            self.log("info", f"Reversal trigger timeout for {rt.pair} — resetting for the next {s.timeframe} slot.", s)
            self._reset(s, rt)
            self._set(s, "waiting", "Reversal trigger timed out — waiting for the next slot.")
            return
        try:
            series = await candle_api.get_candles(rt.pair, trigger_tf, 3)
        except Exception as exc:
            self.log("error", f"{trigger_tf} trigger fetch failed for {rt.pair}: {exc}", s)
            return
        if len(series) < 2:
            return
        candle = series[-2]
        candle_id = str(candle.get("time") or candle.get("timestamp") or candle.get("open_time") or candle)
        if candle_id == rt.last_trigger_candle:
            return
        ...
        if timestamp > 10_000_000_000:
            timestamp /= 1000
        candle_open = datetime.fromtimestamp(timestamp, IST)
        if rt.slot and candle_open < rt.slot:
            return

        direction = candle_side(candle)
        colour = "GREEN" if direction == "buy" else "RED" if direction == "sell" else "FLAT"
        if not rt.seen_green_trigger and direction == "buy":
            action = "Green candle accepted — starting sequence, waiting for Red"
        elif not rt.seen_green_trigger:
            action = "ignored — waiting for a Green candle to start the sequence"
        else:
            action = "checking for confirming Red close"
        self.log(
            "signal",
            f"Fetching ({trigger_tf}) {candle['time']} - OHLC {candle['open']}/{candle['high']}/"
            f"{candle['low']}/{candle['close']} - {colour} ({action}).",
            s,
        )
        if direction == "buy":
            rt.seen_green_trigger = True
            self._set(s, "trigger_wait", f"{rt.pair}: Green {trigger_tf} candle closed — waiting for Red close.")
        elif direction == "sell" and rt.seen_green_trigger:
            self.log(
                "signal",
                f"{rt.pair}: ({trigger_tf}) {candle['time']} - OHLC {candle['open']}/{candle['high']}/"
                f"{candle['low']}/{candle['close']} - RED (Entry Punch Confirmed).",
                s,
            )
            self.log("signal", f"{rt.pair}: Red {trigger_tf} candle closed — executing SELL at {candle['close']}.", s)
            await self._place(s, rt, Decimal(str(candle["close"])))
        else:
    ...
    async def _place(self, s: Strategy, rt: Runtime, price: Decimal) -> None:
        ...
        if not self._inr_pairs:
            try:
                ...
                self.log("error", f"INR instrument list unavailable: {exc}", s)
        if self._inr_pairs and pair not in self._inr_pairs:
            self.log("error", f"{pair} is not tradable with INR margin — slot skipped.", s)
            rt.phase, rt.slot, rt.pair = "waiting", None, None
            self._set(s, "waiting", "Selected coin has no INR-margin contract — waiting for the next slot.")
            return
        ...
        if trade.live_enabled():
            try:
                ...
                self.log("error", f"INR wallet balance unavailable: {exc}", s)
                self._reset(s, rt)
            ...
            if capital < LIVE_MARGIN_MINIMUM:
                self.log(
                    "error",
                ...
                self._reset(s, rt)
                self._set(s, "waiting", "Live wallet margin is too low for a safe trade — waiting for the next slot.")
                return
            if free_capital <= 0:
                self.log("error", f"{pair} skipped live order: free INR futures margin is zero or negative.", s)
                self._reset(s, rt)
                self._set(s, "waiting", "Free futures margin is zero — waiting for the next slot.")
                return
        ...
        try:
            ...
            if instrument:
                leverage = trade.max_leverage_for(instrument, leverage)
                quantity = trade.order_quantity(capital, price, instrument, leverage, rate)
        except Exception as exc:
        ...
        if trade.live_enabled() and (not instrument or quantity <= 0):
            ...
            self._reset(s, rt)
            self._set(s, "waiting", "Coin not tradable on INR margin — waiting for the next slot.")
            return

        entry = float(price)
        tp_pct = 5.0 if s.rule_set == "highest_mover_sell" else s.tp_pct
        sl_pct = None if s.rule_set == "highest_mover_sell" else s.sl_pct
        tp, sl = tp_sl_for(side, entry, tp_pct, sl_pct)

        market_entry = s.order_type == "market"
        if trade.live_enabled():
            try:
                if capital <= 0:
                    raise trade.CoinDcxError("INR futures wallet has no free margin")
                order = await send_order_with_retry(market_entry, pair, side, quantity, leverage, price)
                rt.order_id = str((order or {}).get("id") or "")
                if market_entry:
                    rt.position_id = str((order or {}).get("position_id") or "")
                    if not rt.position_id:
                        position = await trade.find_open_position(pair, side)
                        rt.position_id = str((position or {}).get("id") or (position or {}).get("position_id") or "")
                    if not rt.position_id:
                        self.log(
                            "error",
                            f"LIVE market order on {pair} has no unique position ID after recovery; "
                            "TP/SL and exit safety cannot be established.",
                            s,
                        )
                        self._reset(s, rt)
                        self._set(s, "error", "Market order placed but position ID could not be recovered safely.")
                        return
                order_kind = "market" if market_entry else "limit"
                self.log("trade", f"LIVE {side.upper()} {order_kind} order placed on {pair} @ {entry} ({leverage}x).", s)
            except Exception as exc:
                hint = ""
                if "404" in str(exc):
                    hint = (
                ...
                self.log("error", f"LIVE {('market' if market_entry else 'limit')} order rejected after retries: {exc}{hint}", s)
                self._reset(s, rt)
                self._set(s, "error", f"Order rejected: {exc}")
                return
        else:
            order_kind = "market" if market_entry else "limit"
            self.log("trade", f"PAPER {side.upper()} {order_kind} order on {pair} @ {entry} · qty {quantity or 'n/a'} · {leverage}x.", s)
        ...
        await self._insert_trade(payload)

        rt.trade_id = record.id
        rt.entry, rt.tp, rt.sl = entry, tp, sl
        rt.quantity, rt.leverage, rt.capital = float(quantity), leverage, float(capital)
        ...
        self.log(
            "trade",
            f"{side.upper()} limit {pair} @ {entry:.8f} · TP {tp:.8f} ({s.tp_pct}%) · "
            f"SL {price_label(sl)} ({s.sl_pct}%) · {leverage}x · ₹{float(capital):,.0f} · "
    ...
    async def _order_snapshot(self, s: Strategy, rt: Runtime) -> dict[str, Any] | None:
        ...
        try:
            return await trade.order_status(rt.order_id)
        except Exception as exc:
            self.log("error", f"Order status check failed: {exc}", s)
            return None
    ...
    async def _is_filled(self, s: Strategy | None, rt: Runtime) -> bool:
        if trade.live_enabled() and rt.order_id:
            ...
            rt.last_order_check = loop_now
            try:
                status = await trade.order_status(rt.order_id) or {}
            except Exception as exc:
                if s is not None:
                    self.log("error", f"Order status check failed: {exc}", s)
                return False
            ...
            if state == "partially_filled":
                rt.position_id = str(status.get("position_id") or rt.position_id or "")
                return filled_quantity(status, 0.0) > 0
            return False
    ...
    async def _on_filled(self, s: Strategy, rt: Runtime, avg_price: float, filled_qty: float) -> None:
        if trade.live_enabled() and not rt.position_id:
            position = await trade.find_open_position(rt.pair or "", rt.side or "sell")
            rt.position_id = str((position or {}).get("id") or (position or {}).get("position_id") or "")
            if not rt.position_id:
                self.log(
                    "error",
                    f"LIVE limit fill on {rt.pair} has no unique position ID after recovery; "
                    "TP/SL and exit safety cannot be established.",
                    s,
                )
                self._reset(s, rt)
                self._set(s, "error", "Limit order filled but position ID could not be recovered safely.")
                return
        ...
        if trade.live_enabled() and rt.position_id:
            try:
                ...
                self.log("trade", f"TP/SL attached: TP {price_label(rt.tp)}, SL {price_label(rt.sl)}.", s)
            except Exception as exc:
                self.log("error", f"TP/SL attach failed: {exc}", s)
        side = (rt.side or "sell").upper()
        s.open_pair, s.open_side, s.entry_price, s.tp_price, s.sl_price = rt.pair, rt.side, rt.entry, rt.tp, rt.sl
        self.log("trade", f"Limit order FILLED on {rt.pair} @ {rt.entry:.8f} — position live.", s)
    ...
    async def _finalize_partial_fill(self, s: Strategy, rt: Runtime, avg_price: float, qty: float) -> None:
        if trade.live_enabled() and rt.order_id:
            try:
                ...
                self.log("error", f"Cancelling the unfilled remainder failed: {exc}", s)
        if qty <= 0:
            self.log("info", f"{rt.pair}: partial fill resolved to zero quantity — treated as unfilled.", s)
            await self._update_trade(rt.trade_id, {
                "status": "cancelled",
                "closed_at": now_ist().isoformat(timespec="seconds"),
            })
            self._reset(s, rt)
            self._set(s, "waiting", "Order never filled — cancelled, waiting for the next slot.")
            return
        self.log(
            "trade",
        ...
        await self._on_filled(s, rt, avg_price, qty)

    async def _cancel_unfilled(self, s: Strategy, rt: Runtime) -> None:
        if trade.live_enabled() and rt.order_id:
            try:
                await trade.cancel_order(rt.order_id)
            except Exception as exc:
                self.log("error", f"Order cancel failed: {exc}", s)
        self.log(
            "info",
            f"Limit order on {rt.pair} was not filled inside {order_window(s.timeframe)}s — cancelled, "
