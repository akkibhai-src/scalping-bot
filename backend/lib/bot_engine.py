...
class BotEngine:
    ...
    async def _prescan(
        ...
        detail = (
            ...
            f"Top 4 scanned — reading the {s.timeframe} candle at the slot."
        )
    ...
    async def _select(
        ...
        for pair in rt.candidates:
            ...
            if not await self._inr_instrument(
                pair
            ):
                self.log(
                    "signal",
    ...
    async def _select_highest_mover(
        ...
        for pair in rt.candidates:
            ...
            if (
                not closed
                or not await self._inr_instrument(
                    pair
                )
            ):
                continue
                ...
                if market_entry:
                    ...
                    if not rt.position_id:
                        ...
                        self._set(
                            s,
                            "error",
                            "Market order placed but position ID could not be recovered safely.",
                        )
    ...
    async def _on_filled(
        ...
        if (
            ...
            if not rt.position_id:
                ...
                self._set(
                    s,
                    "error",
                    "Limit order filled but position ID could not be recovered safely.",
                )
        ...
        if (
            ...
            try:
                ...
                self.log(
                    "error",
                    f"TP/SL tick rounding failed for {rt.pair}: {exc}",
                    s,
                )
