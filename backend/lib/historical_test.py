...
async def run_historical_test(strategy: Strategy, target: datetime) -> dict[str, Any]:
    ...
    selected = movers[:1] if strategy.rule_set == "highest_mover_sell" else movers[:4]
    if strategy.rule_set == "top4_5m_reversal_short":
    ...
    for item in selected:
        ...
        if strategy.rule_set == "highest_mover_sell":
            side = "sell"
        if side:
