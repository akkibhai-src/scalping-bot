...
from lib.bot_engine import engine, get_engine, now_ist
...
async def user_engine(request: Request):
    user_id = str(request.session.get("user_id") or "__anonymous__").strip().lower()
...
async def create_strategy(body: StrategyCreate, request: Request) -> Strategy:
    ...
    strategy = Strategy(
        **body.model_dump(),
        owner_id=str(request.session.get("user_id") or "admin").lower(),
...
async def bot_ws(websocket: WebSocket) -> None:
    ...
    owner_id = str(websocket.session.get("user_id") or "__anonymous__").strip().lower()
